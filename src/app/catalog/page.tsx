...
import { GroupView } from '@/components/catalog/group-view'
import { ProductDetail } from '@/components/catalog/product-detail'
import { motion, AnimatePresence } from 'framer-motion'
import { Home, ChevronLeft, Moon, Sun, Loader2, LogIn } from 'lucide-react'
...
export default function PublicCatalogPage() {
  ...
  // Data — public APIs (GET routes don't require auth)
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      {/* Top nav — same style but no admin/logout */}
      <header className="sticky top-0 z-40 bg-background/85 backdrop-blur-md border-b border-border/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 h-16 flex items-center justify-between gap-4">
          ...
          <button onClick={goHome} className="flex items-center gap-2.5 group">
            <div
              className="w-9 h-9 rounded-full bg-[oklch(0.32_0.035_45)] flex items-center justify-center text-cream font-display text-base"
              style={{ backgroundColor: 'oklch(0.32 0.035 45)', color: 'oklch(0.96 0.015 80)' }}
            >
              B
            </div>
